...
fn is_diagonal(a: Point, b: Point) -> bool {
    (b.y as i64 - a.y as i64).abs() == (b.x as i64 - a.x as i64).abs()
}

fn is_horizontal_or_vertical_or_diagonal(a: Point, b: Point) -> bool {
    if is_horizontal(a,b) || is_vertical(a,b) || is_diagonal(a,b) { 
        true
    } else {
        false
    }
...
fn main() -> Result<(), Box<dyn Error + 'static + Send + Sync>> {
    ...
    let points: Vec<(Point,Point)> = lines
        .map(|line| parse_points(&line.unwrap()).unwrap() )
        .filter(|(a,b)| is_horizontal_or_vertical_or_diagonal(*a,*b))
        .collect();
    println!("{}", points.len());
    ...
    for (point_a, point_b) in points {
       if is_vertical(point_a, point_b) {
           ...
           let dx: isize = if point_a.x < point_b.x { 1 } else { -1 };
           let dy: isize = if point_a.y < point_b.y { 1 } else { -1 };
           let Point { mut x, mut y} = point_a;
           loop {
               if x == point_b.x && y == point_b.y { break; }
               map.entry(Point { x: x, y: y } )
                   .and_modify(|e| { *e += 1 })
                   .or_insert(1);
               println!("({}.{})", x, y);
...
mod test {
    ...
    #[test]
    fn point_is_diagonal() {
        assert!(is_diagonal(Point { x: 101, y: 150 }, Point { x: 102, y: 151 }));
    }
}
